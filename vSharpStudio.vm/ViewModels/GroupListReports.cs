...
namespace vSharpStudio.vm.ViewModels
{
    ...
    public partial class GroupListReports : ITreeModel, ICanAddSubNode, ICanGoRight, ICanGoLeft, INodeGenSettings, IEditableNodeGroup
    {
        ...
        partial void OnSortTypeChanged() { this.ListReports.Sort((int)this.SortType); }
        private void Init()
        {
            OnSortTypeChanged();
        ...
        public override ITreeConfigNode NodeAddNewSubNode(ITreeConfigNode? node_impl = null)
        {
            ...
            if (this.ListReports.Count > 0)
            {
                node.SortingValue = this.ListReports[this.ListReports.Count - 1].SortingValue + 1;
            }
            else
            {
                node.SortingValue = 1;
