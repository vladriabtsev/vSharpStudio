...
namespace vSharpStudio.vm.ViewModels
{
    public partial class DataTypeValidator
    {
        ...
        public DataTypeValidator()
        {
            ...
            #region ObjectGuid
            this.RuleFor(p => p.ObjectRef.RefConfigObjectGuid).Must((p, y) =>
            {
                if (p.DataTypeEnum == EnumDataType.ENUMERATION && string.IsNullOrWhiteSpace(p.ObjectRef.RefConfigObjectGuid))
                {
                    return false;
                }
            ...
            this.RuleFor(p => p.ObjectRef.RefConfigObjectGuid).Must((p, y) =>
            {
                if (p.DataTypeEnum == EnumDataType.CATALOG && string.IsNullOrWhiteSpace(p.ObjectRef.RefConfigObjectGuid))
                {
                    return false;
                }
            ...
            this.RuleFor(p => p.ObjectRef.RefConfigObjectGuid).Must((p, y) =>
            {
                if (p.DataTypeEnum == EnumDataType.DOCUMENT && string.IsNullOrWhiteSpace(p.ObjectRef.RefConfigObjectGuid))
                {
                    return false;
                }
