...
namespace vSharpStudio.vm.ViewModels
{
    ...
    public partial class GroupListProperties : IListProperties, ITreeModel, ICanAddSubNode, ICanGoRight, ICanGoLeft, INodeGenSettings, IEditableNodeGroup, IRoleAccess
    {
        ...
        partial void OnSortTypeChanged() { this.ListProperties.Sort((int)this.SortType); }
        private void Init()
        {
            OnSortTypeChanged();
        ...
        public override ITreeConfigNode NodeAddNewSubNode(ITreeConfigNode? node_impl = null)
        {
            ...
            if (this.ListProperties.Count > 0)
            {
                node.SortingValue = this.ListProperties[this.ListProperties.Count - 1].SortingValue + 1;
            }
            else
            {
                node.SortingValue = 1;
