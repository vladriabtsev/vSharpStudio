#nullable enable
namespace CustomTypesGrpc // CustomTypes.tt Line: 9, called from GrpcGeneratorSettings.partial.cs Line: 205
{
    using System;
    using vPlugins;
    ...
    {
        public static void Translate(object? prms, Google.Protobuf.Collections.RepeatedField<parameter> lst)
        {
            if (prms == null)
                return;
            ...
            foreach (var nam in dic.Keys)
            {
                ...
                if (val == null)
                {
                    p = new parameter() { Name = nam, Type = param_type.Null };
                }
                else
                {
                    p = new parameter() { Name = nam, Val = val.ToString(), Type = GetParamType(val) };
                }
                lst.Add(p);
            }
        }
        private static System.Collections.Generic.Dictionary<string, param_type>? dic;
        private static param_type GetParamType(object val)
        {
            if (dic == null)
            {
                dic = new();
                dic[typeof(bool).Name] = param_type.Bool;
                dic[typeof(byte).Name] = param_type.Byte;
                dic[typeof(sbyte).Name] = param_type.Sbyte;
                dic[typeof(char).Name] = param_type.Char;
                dic[typeof(decimal).Name] = param_type.Decimal;
                dic[typeof(double).Name] = param_type.Double;
                dic[typeof(float).Name] = param_type.Float;
                dic[typeof(int).Name] = param_type.Int;
                dic[typeof(uint).Name] = param_type.Uint;
                dic[typeof(long).Name] = param_type.Long;
                dic[typeof(ulong).Name] = param_type.Ulong;
                dic[typeof(short).Name] = param_type.Short;
                dic[typeof(ushort).Name] = param_type.Ushort;
                dic[typeof(string).Name] = param_type.String;
                dic[typeof(Guid).Name] = param_type.Guid;
                dic[typeof(TimeOnly).Name] = param_type.Timeonly;
                dic[typeof(DateOnly).Name] = param_type.Dateonly;
                dic[typeof(DateTime).Name] = param_type.Datetime;
                dic[typeof(DateTimeOffset).Name] = param_type.Datetimeoffset;
            }
            return dic[val.GetType().Name];
        }
    }
}
