...
namespace vSharpStudio.ViewModels
{
    ...
    public class EditorRolesTreeVm : ObservableObject, ITreeModel
    {
        ...
        public EditorRolesTreeVm(ConfigNodesCollection<Role> roles, Model model)
        {
            this.model = model;
            this.Node = this.model;
            EditorRolesTreeVm.roles = roles;
            this.ListRoleColumns = new List<EditorRoleColumnVm>();
        }
        public EditorRolesTreeVm(ITreeConfigNode node)
        {
            this.Node = node;
            this.ListRoleColumns = new List<EditorRoleColumnVm>();
            Debug.Assert(EditorRolesTreeVm.roles != null);
            foreach (var role in EditorRolesTreeVm.roles)
        ...
        private readonly List<EditorRolesTreeVm> children = new List<EditorRolesTreeVm>();
        public IEnumerable GetChildren(object parent)
        {
            ITreeConfigNode node = parent == null ? this.Node : ((EditorRolesTreeVm)parent).Node;
            ...
            foreach (var t in node.GetListChildren())
            {
                ...
                if (parent == null && (tt.Name == "Common" || tt.Name == "Enumerations" || tt.Name == "Journals"))
                    ...
                    continue;
                var nd = new EditorRolesTreeVm(tt);
                res.Add(nd);
        ...
        {
            var p = (EditorRolesTreeVm)parent;
